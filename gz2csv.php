...
$dirx = dirname(__FILE__)."/gz_in";
$target = dirname(__FILE__)."/gz2_csv_out"; 
...
//        if($i==$break)exit;
        ...
        if($ext == 'gz'){
                $file = $path; 
                $gzfile = $target_path; 
                $out_file_name = str_replace('.gz', '', $target_path);
                $out_file_name = str_replace($dirx, $target, $out_file_name);
